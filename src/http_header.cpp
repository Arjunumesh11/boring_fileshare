...
std::string http_header::make_header(std::string filename, int file_size, std::string disposition)
{
    std::ostringstream header;
    header << "HTTP/1.1 200 OK\r\n";
    header << "Content-Type: " << http_header::get_contenttype(filename) << "\r\n";
    header << "Content-Disposition: " << disposition << "; filename =\"" << http_header::get_filename(filename) << "\"\r\n";
